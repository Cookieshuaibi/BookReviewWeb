                ...
                <a href="{{ url('/notifications/' . $notification->id) }}"
                   class="list-group-item list-group-item-action flex-column align-items-start">
                    <div class="d-flex w-100 justify-content-between">
                        <h5 class="mb-1">
                            {{ $notification->data['reviews_user_username'] }} has reviewed your book {{ $notification->data['reviews_book_title'] }}
                        </h5>
                        @if(!$notification->read_at)
                            <span class="badge bg-danger rounded-pill">New</span>
                        @endif
                    </div>
                    <small class="text-muted mb-1">{{ $notification->data['reviews_content'] }}</small>
